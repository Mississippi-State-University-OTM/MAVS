...
class OakDCamera : public Sensor {
public:
	/// Constructor
	OakDCamera();
	...
	/// Copy constructor
	OakDCamera(const OakDCamera& cam);
	...
	void FreePose() { center_cam_.FreePose(); }

	/// Return true if the camera display is open
	bool DisplayOpen() { return center_cam_.DisplayOpen(); }
	...
	Pose GetPose() { return center_cam_.GetPose(); }

	int GetWidth() { return center_cam_.GetWidth(); }
	int GetHeight() { return center_cam_.GetHeight(); }
	int GetDepthImageWidth() { return left_cam_.GetWidth(); }
	int GetDepthImageHeight() { return left_cam_.GetHeight(); }

	float* GetImageBuffer() { return center_cam_.GetImageBuffer(); }

	int GetBufferSize() { return center_cam_.GetBufferSize();}
	...
	RgbCamera* GetCamera() { return &center_cam_; }
